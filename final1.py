...
cred = credentials.Certificate('./android-app-4a201-firebase-adminsdk-6lwoo-c1098848b8.json')
firebase_admin.initialize_app(cred, {
    'databaseURL' : 'https://android-app-4a201.firebaseio.com'
})
...
def readIntensity():
	...
	root.child('LabStatus').update({'Intensity' : float(format(lightLevel,'.2f'))})
 	time.sleep(0.5)
...
def roomTemp():
	...
	if humidity is not None and temperature is not None:
		...
		root.child('LabStatus').update({'Humidity' : float(humidity)})
		root.child('LabStatus').update({'Temp' : float(temperature)})
	else:
...
def motor():
	x=bool(db.reference('motor_pump').get())
	if x==1 :
...
while True:
	...
	lighttop()
	lightmain()
	'''root.child('LabStatus').update({'Intensity' : 1799})
	print("------------------------------------------")
